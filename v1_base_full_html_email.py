...
def generate_html_report(entries: List[AnomalyEntry], filename: str):
    ...
    <head>
        ...
        <style>
            ...
            /* Add new styles for email success state */
            .email-success {
                text-align: center;
                padding: 20px;
                color: #28a745;
        ...
        <script>
            ...
            const EMAILJS_CONFIG = {
                publicKey: 'QpsKt2E5aUIPx4DVg',  // Replace with your actual public key
                serviceId: 'service_hxqj7ka',  // Replace with your actual service ID
                templateId: 'template_dzzpsso' // Replace with your actual template ID
            };
            ...
            // Function to compress image to stay under 50KB
            async function compressImage(dataUrl) {
                return new Promise((resolve, reject) => {
                    ...
                    img.onload = function() {
                        ...
                        // Try different quality levels until we get under 50KB
                        const qualities = [0.7, 0.5, 0.3, 0.2];
                        let currentQuality = 0;
                        
                        function tryCompress() {
                            if (currentQuality >= qualities.length) {
                                // If we've tried all qualities and still too large, reduce dimensions
                                const finalCanvas = document.createElement('canvas');
                                finalCanvas.width = Math.round(width * 0.7);
                                finalCanvas.height = Math.round(height * 0.7);
                                const finalCtx = finalCanvas.getContext('2d');
                                finalCtx.fillStyle = '#ffffff';
                                finalCtx.fillRect(0, 0, finalCanvas.width, finalCanvas.height);
                                finalCtx.drawImage(canvas, 0, 0, finalCanvas.width, finalCanvas.height);
                                resolve(finalCanvas.toDataURL('image/jpeg', 0.2));
                                return;
                            }
                            ...
                            if (decodedSize <= 45000) { // Leave some margin for other variables
            ...
            window.openEmailClient = async function(idx, event) {
                if (event) { event.preventDefault(); event.stopPropagation(); }
                try {
                    ...
                    if (!modal || !modalBody) {
                        throw new Error('Email modal elements not found');
                    }

                    // Show loading state in modal
                    modalBody.innerHTML = `
                        <div class="screenshot-loading">
                            <p>Preparing screenshot with charts... Please wait...</p>
                            <p>This may take a few seconds...</p>
                        </div>
                    `;
                    modal.classList.add('show');

                    // Store original states of all accordions
                    const originalStates = {
                        mainAccordion: false,
                        rootCauses: []
                    };

                    // Expand main anomaly accordion
                    const mainContent = anomalyContainer.querySelector('.accordion-content');
                    if (mainContent) {
                        originalStates.mainAccordion = mainContent.classList.contains('show');
                        mainContent.classList.add('show');
                    }

                    // Expand all root cause accordions and ensure charts are rendered
                    const rootCauses = anomalyContainer.querySelectorAll('.root-cause');
                    for (const rc of rootCauses) {
                        const content = rc.querySelector('.root-cause-content');
                        if (content) {
                            originalStates.rootCauses.push({
                                index: Array.from(rootCauses).indexOf(rc),
                                wasExpanded: content.classList.contains('show')
                            });
                            content.classList.add('show');
                            
                            // Find and render the chart if it exists
                            const chartCanvas = content.querySelector('canvas');
                            if (chartCanvas) {
                                const chartId = chartCanvas.id;
                                const chart = window.chartInstances[chartId];
                                if (chart) {
                                    // Force chart update and wait for it to render
                                    chart.update('none');
                    ...
                    // Capture screenshot with all content expanded and charts rendered
                    const screenshot = await html2canvas(anomalyContainer, {
                        scale: 1,
                        useCORS: true,
                        logging: false,
                        ...
                        onclone: function(clonedDoc) {
                            ...
                            const clonedMainContent = clonedDoc.querySelector('.accordion-content');
                            if (clonedMainContent) {
                                clonedMainContent.style.display = 'block';
                                // Remove unnecessary styles and elements
                                const tables = clonedMainContent.querySelectorAll('table');
                            ...
                            // Ensure all root causes and charts are visible
                            const clonedRootCauses = clonedDoc.querySelectorAll('.root-cause-content');
                            clonedRootCauses.forEach(content => {
                                ...
                                if (chartCanvas) {
                                    ...
                                    if (chart) {
                                        ...
                                        if (container) {
                                            ...
                                            chartCanvas.height = 300; // Fixed height for consistency
                                            chart.resize();
                                        }
                                    }
                                }
                    ...
                    if (mainContent) {
                        if (!originalStates.mainAccordion) {
                    ...
                    rootCauses.forEach((rc, index) => {
                        const content = rc.querySelector('.root-cause-content');
                        if (content && !originalStates.rootCauses[index]?.wasExpanded) {
                            content.classList.remove('show');
                        }
                    });

                    // Compress the screenshot
                    const originalDataUrl = screenshot.toDataURL('image/png');
                    const compressedDataUrl = await compressImage(originalDataUrl);
                    
                    // Update modal with form and compressed screenshot preview
                    modalBody.innerHTML = `
                        <form class="email-form" onsubmit="sendEmail(event, '${compressedDataUrl}')">
                            <div class="email-form-group">
                                <label for="emailTo">To:</label>
                                <input type="email" id="emailTo" required placeholder="Enter recipient email">
                            </div>
                            <div class="email-form-group">
                                <label for="emailSubject">Subject:</label>
                                <input type="text" id="emailSubject" required value="AWS Cost Anomaly Alert">
                            </div>
                            <div class="email-form-group">
                                <label for="emailBody">Message:</label>
                                <textarea id="emailBody" required rows="4">Please find attached the details of the AWS Cost Anomaly, including cost usage graphs.</textarea>
                            </div>
                            <div class="screenshot-preview">
                                <h4>Screenshot Preview (Optimized for Email):</h4>
                                <img src="${compressedDataUrl}" alt="Anomaly Details Screenshot">
                                <p class="screenshot-info">Image has been optimized to meet EmailJS size requirements while maintaining readability. Includes cost usage graphs.</p>
                            </div>
                            <div class="email-form-actions">
                                <button type="button" class="email-btn email-btn-secondary" onclick="closeEmailModal()">Cancel</button>
                                <button type="submit" class="email-btn email-btn-primary">Send Email</button>
                            </div>
                        </form>
                    `;
                    ...
                    if (modalBody) {
                        modalBody.innerHTML = `
                            <div class="screenshot-error">
                                Failed to prepare email: ${err.message}
                            </div>
                            <div class="email-form-actions">
                                <button class="email-btn email-btn-secondary" onclick="closeEmailModal()">Close</button>
                            </div>
                        `;
                    } else {
            ...
            window.sendEmail = async function(event, screenshotDataUrl) {
                event.preventDefault();
                try {
                    ...
                    const response = await emailjs.send(
                        ...
                        EMAILJS_CONFIG.templateId,
                        {
                            ...
                            from_name: 'AWS Cost Anomaly Alert',
                            reply_to: to
                        }
                    );
                    ...
                    if (response.status === 200) {
                        modalBody.innerHTML = `
                            <div class="email-success">
                                ...
                                <p>The anomaly details have been sent to ${to}</p>
                                <div class="email-form-actions">
                                    <button class="email-btn email-btn-secondary" onclick="closeEmailModal()">Close</button>
                                </div>
                            </div>
                    ...
                    if (errorMessage.includes('EmailJS is not configured')) {
                        errorMessage = `
                            <h4>EmailJS Not Configured</h4>
                            <p>Please follow these steps to set up EmailJS:</p>
                            <ol>
                                <li>Sign up for a free account at <a href="https://www.emailjs.com/" target="_blank">EmailJS.com</a></li>
                                <li>Create an email service (Gmail, Outlook, etc.)</li>
                                <li>Create an email template with these variables:
                                    <ul>
                                        <li>{{to_email}} - Recipient's email</li>
                                        <li>{{subject}} - Email subject</li>
                                        <li>{{message}} - Email message</li>
                                        <li>{{screenshot}} - Anomaly screenshot (use as image source)</li>
                                        <li>{{from_name}} - Sender name</li>
                                        <li>{{reply_to}} - Reply-to email</li>
                                    </ul>
                                </li>
                                <li>In your EmailJS template, use the screenshot like this:
                                    <pre>&lt;img src="{{screenshot}}" alt="Anomaly Details" style="max-width: 100%; height: auto;"&gt;</pre>
                                </li>
                                <li>Get your credentials and update them in the code:
                                    <ul>
                                        <li>Public Key</li>
                                        <li>Service ID</li>
                                        <li>Template ID</li>
            ...
            // Add event listener for modal close on escape key
            ...
            // Add some styles for the screenshot info
        ...
        <div id="emailModal" class="email-modal">
            <div class="email-modal-content">
                <div class="email-modal-header">
                    <div class="email-modal-title">Send Anomaly Details</div>
                    <div class="email-modal-close" onclick="closeEmailModal()">&times;</div>
                </div>
    ...
    # Fix the regex pattern in the JavaScript code
    html = html.replace(r'\d+', r'\\d+')  # Fix the escape sequence
...
if __name__ == "__main__":
    try:
        ...
        html_file = generate_html_report(anomalies, "aws_anomalies_detailed.html")
        ...
        print("1. Sign up for a free EmailJS account at https://www.emailjs.com/")
        print("2. Create an email service and template")
        print("3. Replace the EmailJS credentials in the HTML file:")
        print("   - YOUR_PUBLIC_KEY")
        print("   - YOUR_SERVICE_ID")
        print("   - YOUR_TEMPLATE_ID")
        print("\nPress Ctrl+C to exit...")
        
        # Keep the program running
        while True:
            asyncio.run(asyncio.sleep(1))  # Fix the coroutine warning
